# MarketBW — документация

> Карта агента: **plans/** (Grace 2).



Документация **для людей**: где что лежит, как течёт запрос, где менять поведение. Карта агента — **`plans/`**.

> **Workflow агента** — Grace 2 (`plans/`, см. раздел «Правила агента»), не marketplace skills `grace-*`.
...
- Версия релиза: файл **`VERSION`** в корне; subject коммита **`X.Y.Z: описание`** (`.cursor/rules/agent-rules.mdc`).

---

## Правила агента

- **Grace 2:** `.cursor/rules/grace-2-framework.mdc`, `.kilocode/rules/rules.md`; skills `mode-*` в `.cursor/skills/`, `.kilocode/skill/`.
- **Ops:** `.cursor/rules/agent-rules.mdc` — `docs/HISTORY.md`, этот файл, автокоммит и push, PowerShell, русский.
