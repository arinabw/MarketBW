...
> Каноническая семантическая карта для ИИ-агентов и разработчиков (правила Cursor: читать при старте после **`docs/HISTORY.md`**).

Документ для **ИИ-агентов и разработчиков**: где что лежит, как течёт запрос, где менять поведение.

> **GRACE-артефакты** (Graph-RAG Anchored Code Engineering): `grace/requirements/requirements.xml`, `grace/technology/technology.xml`, `grace/plan/development-plan.xml`, `grace/verification/verification-plan.xml`, `grace/knowledge-graph/knowledge-graph.xml`. Они — каноническая карта модулей, контрактов, потоков данных и плана верификации. Обзор каталога: **`grace/README.md`**.
...
| `docs/` | **`docs/AGENTS.md`**, **`docs/HISTORY.md`**; прочая Markdown-документация |
| `grace/` | XML GRACE по этапам ЖЦ: `requirements/`, `plan/`, `technology/`, `verification/`, `knowledge-graph/` |
...
- Версия релиза: файл **`VERSION`** в корне репозитория и правило **`.cursor/rules/git-version-commit.mdc`**.
