...
Если после **отправки формы** вас выкидывает на **логин**, хотя страница открывалась под авторизацией: обычно это **потеря cookie сессии** — за HTTPS-прокси (Traefik) PHP должен видеть **`X-Forwarded-Proto: https`** (в образе Nginx передаёт заголовок в PHP-FPM), чтобы выставить **`Secure`** на cookie; при **www** и **apex** одного домена задайте **`SESSION_COOKIE_DOMAIN=.ваш-домен.ru`** в окружении контейнера. Редиректы после сохранения учитывают **`BASE_PATH`**, если сайт в подкаталоге.
